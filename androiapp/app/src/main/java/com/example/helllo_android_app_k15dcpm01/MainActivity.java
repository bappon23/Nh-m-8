...
public class MainActivity extends AppCompatActivity {
    ...
    private ArrayAdapter<Product> createProductListAdapter(List<Product> productList) {
        return new ArrayAdapter<>(MainActivity.this, android.R.layout.simple_list_item_1, productList);
    }
            ...
            public void onResponse(Call<List<Product>> call, Response<List<Product>> response) {
                if (response.isSuccessful()) {
                    if (listView.getAdapter() == null) {
                        ...
                        ArrayAdapter<Product> adapter = createProductListAdapter(productList);
                        listView.setAdapter(adapter);
                    } else {
                        List<Product> productList = response.body();
                        ((ArrayAdapter<Product>) listView.getAdapter()).clear();
                        ((ArrayAdapter<Product>) listView.getAdapter()).addAll(productList);
                    }
                } else {
